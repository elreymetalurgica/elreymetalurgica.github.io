document.addEventListener('DOMContentLoaded', () => {
    ...
    const showHTML = () => {
        ...
        allProducts.forEach(product => {
            ...
            // Limpiar el precio y convertirlo a número (eliminar símbolo $ y caracteres no numéricos)
            const priceValue = parseFloat(product.price.replace(/[^0-9.-]+/g, "")); // Eliminar caracteres no numéricos
        ...
        // Usamos `Intl.NumberFormat` para formatear el número con separadores de miles, sin redondear el valor
        const formattedTotal = new Intl.NumberFormat('es-AR', { 
            minimumFractionDigits: 0, // No mostrar decimales
            maximumFractionDigits: 0  // No mostrar decimales
        }).format(total); // Formatear el total con separadores de miles
        
        // Mostrar el total con el formato adecuado
